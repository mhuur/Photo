...
- **Édition in-place dans la preview** — pour éditer un texte rich-text (CGV section, signature, conditions paiement, préambule CGV), wrapper la zone dans `.dp-rich-zone` avec un bouton-menu flottant : `<div class="dp-rich-zone" data-rich-field="X"><button class="dp-rich-menu-btn" onclick="event.stopPropagation();textEditorOpen('X')">${ICO_SZ('moreVertical', 14)}</button>{contenu}</div>` (icône Lucide, plus jamais le caractère `⋮` — arbitrage 2026-07-14). Le bouton apparaît au hover (opacity 0→1), masqué `@media print` et dans `.view-devis-snapshot` (PDF nickel). Pour les sections CGV, modal d'édition dédié `cgvSectionEditModal` (titre + body + ordre + suppression).
...
- **Segmented controls `.seg-neon`** — la classe **garde son nom** ; l'option active se lit à son **fond plein azure** (`.warn` ambre pour Débours, `.ok` jade). Règle (arbitrage 2026-07-14) : tout toggle qui pilote une **donnée** (clientèle, h/j, Voiture/Transports, Principal/Débours, Acompte/Avance, Refacturé/Direct) = fill ; tout toggle de **présentation/raffinement** = variante `.violet` (sélection en creux, bordure + texte azure). Il ne reste que 4 `.violet` : « Affichage sur le devis » + les 3 segs d'arrondi (mode/palier/sens).
