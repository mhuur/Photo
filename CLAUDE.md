...
- **Le poids structurel du doc = les snapshots HTML inline** — chaque devis archivé stocke `snapshotHtml` + `snapshotDeboursHtml` + `snapshotMission` (CGV complètes en triple), chaque facture de solde re-embarque le `snapshotHtml` du devis entier, chaque révision garde tout, et la corbeille `S.suivi.bin` conserve les chaînes complètes AVEC snapshots (30 items). Le nettoyage logos ne récupère que des miettes : à l'approche de la limite, la vraie marge est de vider la corbeille, puis décision structurelle (re-render depuis `snapshotMission` / sous-collections / compression).
