...
- **Dépliage au SURVOL, pur CSS** (`.sb-group:hover .sb-group-body`) : replié au repos, déplié au hover, refermé en sortant. `navGroupPinned` (variable module, **jamais dans `S`**) n'est qu'un filet posé au clic pour le **tactile** (aucun `:hover` sur un écran tactile → les sous-onglets seraient inatteignables) et le clavier (`:focus-within`). ⚠ **Ne JAMAIS persister cet état** : passer par `S` + `save()` faisait clignoter « Enregistré » (le flash vient de `saveToCloud`) à chaque dépliage, et polluait le doc Firestore pour une préférence d'affichage. Règle générale : **toute interaction purement visuelle doit rester hors de `S`**.
- **Icônes uniques** : chaque icône n'apparaît qu'UNE fois dans la sidebar, groupes compris (une icône partagée entre un groupe et l'un de ses enfants rend le menu illisible). La pastille « brouillon » est remontée sur l'en-tête du groupe tant qu'il est replié, sinon elle serait invisible.
