...
- **Style visuel : boutons du mock** — la charte néon pour les **boutons d'action** est **révoquée** (refonte timeline devis, mock dans `redesign-timeline-des-devis/`). Nouveau standard = **fills pleins assumés**, classe `.mk-btn` + variante :
  ...
  - `.mk-btn.blue` (`#2f6fd6`, texte blanc) — jalon neutre (« Marquer livré »).
...
**Parcours séquentiel verrouillé** (ordre canonique) : envoi (`dv.envoiTs`) → acceptation → par acompte dans l'ordre : paiement puis facture FA → livraison (`dv.livreeTs`) → facture FS → paiement solde. **Rail** (refonte timeline) : `devisParcoursSteps(devisId)` est la SOURCE UNIQUE de la séquence + statut par étape (`done`/`active`/`upcoming`/`refused`), consommée par le rendu (`suiviStepRowsHtml`) ET le retour arrière (`suiviStepBack`). L'**étape active** du rail porte ses actions inline (plus de bandeau `nextStepHint`, supprimé) et reste le SEUL point d'émission de facture (les factures ne s'affichent en ligne que quand émises). Gardes d'ordre dans `suiviLigneTogglePaiement` (encaissement dans l'ordre, solde exige livraison+FS, annulations en ordre inverse) et `suiviMarkLivree`. `isFactureApplicable(solde)` dérive la chaîne amont COMPLÈTE (acomptes encaissés ET facturés + livrée), pas juste les paiements.
