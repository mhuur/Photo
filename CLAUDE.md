...
- **Mission = formulaire à gauche + récap sticky à droite (`.ms-grid`)** — `rMissionAsideInner()` a **absorbé la toolbar** (`rMissionToolbarInner` supprimé, ainsi que `rMissionModeBar` → `rMissionModeCards`, et le `totalsHtml` mort). Il est ré-rendu en entier par `refreshDevisHeaders()` à chaque frappe ⇒ **il ne doit JAMAIS contenir d'`<input>`** (le focus serait perdu). Ses montants viennent de `totals()` et ses échéances de `t.echeances` (dérivées de `buildEcheanceLines`) — jamais un calcul parallèle. Statut = `validateDevis()`.
  ...
  - ⚠ **Les clés de `sectionKeys` dans `rMS` doivent matcher EXACTEMENT celles rendues par `rDevisAccordion`.** Elles listaient encore des sections disparues (`ajustements`, `modalites`) ; `modalites` étant pré-initialisée à `false` dans `_editSectionOpen`, le test `=== undefined` échouait et **la 1ʳᵉ section ne s'ouvrait jamais**. Bug silencieux resté longtemps ; attrapé au smoke test.
...
- **Label de zone hors d'un `.field`** : utiliser `.field-label-only` (11 px text-muted, même typo que les labels d'inputs — densifiés 13→11 px le 2026-07-14) plutôt qu'un `<h5>`. Le `<h5>` reste pour les vrais sous-titres de section dans une accordion (désormais Space Mono 10 px 700 uppercase `.16em`).
