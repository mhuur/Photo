...
- **Segmented controls `.seg-neon`** — la classe **garde son nom** (14 usages : Mixte/Particulier/Entreprise, Voiture/Transports, Acompte/Avance) mais son halo a sauté en phase 7 : l'option active se lit à son **fond plein**, plus à un glow. La variante `.violet` portait un fuchsia `#c026d3` **hors palette V2** (reste de l'ancien thème) → devenue le toggle **secondaire**, sélection en creux (bordure + texte azure) pour ne pas rivaliser avec les toggles de 1ᵉʳ niveau posés au-dessus.
