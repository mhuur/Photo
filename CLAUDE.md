...
- **Fond photo** : `body` porte la photo de marque (`assets/*.jpg`) sous un voile lourd (`--bg-scrim: .92`). Piloté par `S.prefs.bgPhoto` → `applyBgPhoto()` pose `html[data-bg]` (appelé depuis `render()`, donc couvre boot + merge cloud + live sync). Console : `setBgPhoto("azure-helmet"|"ember-parking"|"jade-portrait"|"none")`. **Aucune image en data URL** (cf. limite Firestore 1 MB) — ce sont des fichiers servis.
...
- [ ] **Phase 1 — Coquille** : sidebar 216 px (wordmark, groupes Space Mono, footer Bugs + Mon compte) + en-tête de page `.v2-page-head` factorisé.
