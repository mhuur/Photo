...
- **Pied de sidebar (`.sb-foot`)** : deux onglets de 1ᵉʳ niveau, « Bugs & suggestions » et « Mon compte » (ce dernier porte l'avatar à la place d'une icône). **Le menu déroulant user n'existe plus** (`userMenuToggle`/`userMenuClose` supprimés en phase 6).
...
- **CGV — foyer d'édition UNIQUE = `rCgvEditorUnified()`** (refonte 2026-07-17). Données inchangées dans `S.mission` (`cgvSections[]` `{id,title,body}` + `cgvPreambule` + `conditionsPaiement` + `signatureBlock`), gabarit reporté par `missionNew`. Un SEUL éditeur, tout visible sur une page (onglet Profil `rPF`) : sections éditées **inline** (`<input>` titre + `<textarea>` corps → `cgvSectionUpdate` qui ne `render()` PAS → focus conservé), ↑/↓ `cgvSectionMoveUp/Down`, suppr `cgvSectionRemove` (undo toast). Préambule / conditions / signature restent **rich-text** → aperçu + un « Modifier » qui ouvre `textEditorOpen` (`rTextEditorModal`, contentEditable, stocke l'innerHTML). L'**aperçu du devis est en LECTURE SEULE** (plus de `.dp-rich-zone`/`⋮` — tous supprimés) : un unique lien « Personnaliser les CGV de ce devis » (`cgvEditorOverlayShow`, réutilise la classe `.dp-cgv-add-btn`, live-only) rouvre `rCgvEditorUnified()` dans l'overlay `rCgvEditorOverlayModal` — injecté **avant** `${rTextEditorModal()}` dans `render()` pour que l'éditeur rich passe au-dessus si ouvert depuis l'overlay. `_cgvEditorOverlay` = transient (jamais dans `S`). Morts et supprimés : `rCgvEditor`, `rDevisSectionCgv`, `rDevisSectionConditions`, `cgvSectionEditModal`/`cgvSectionEditOpen`/`rCgvSectionEditModal` + CSS `.dp-rich-zone`/`.cgv-summary-*`/`.cgv-edit-*`/`.pf-cgv-*`. ⚠ **Ne pas réintroduire d'édition inline des CGV dans l'aperçu.**
- **Charte graphique — variables CSS obligatoires** — toute nouvelle valeur d'espacement, hauteur ou rayon doit utiliser les variables `:root` plutôt qu'un nombre en dur. Cohérence visuelle de l'app, pas d'anarchie.
  ...
  - **Rayons** (alignés maquette, arbitrage 2026-07-14) : `--radius-sm:4px` (pills, contrôles, `.mk-btn`), `--radius-md:4px` (inputs), `--radius-lg:6px` (cards/sections/modals), `--radius-xl:8px` (gros conteneurs). `.seg` = 5px littéral (maquette).
...
### Charte V2 — palette Saintilann (refonte 2026-07, maquette archivée dans `refonte-v2/`)
...
- **Ligne de devis = `devisRowHtml(g, opts)`** — SOURCE UNIQUE, partagée par l'Accueil (`rAC`) et l'Historique (`rSV`). Les deux utilisaient la classe `.ac-row` avec chacune leur markup : passer le grid à 7 colonnes a silencieusement décalé l'Historique. ⚠ Le grid compte **7 colonnes** — toute cellule sautée (ex. pas de bouton de relance sur un devis clos) doit être un `<span></span>` vide, sinon les suivantes se décalent.
- **Forme d'un « groupe » de devis = `_groupFromLignes()`** — `suiviGroupsAll()` ne renvoyait que `{devisId, lignes, statutPrin}` (sans `ref`/`total`/dates), si bien que tout appelant hors Historique (fiche client…) affichait des champs vides. `suiviGroups()` (filtré + trié pour l'Historique) et `suiviGroupsAll()` (brut) en dérivent désormais toutes les deux.
- **Fiche client** : `clientsStats()` calcule EN UNE PASSE (pas par ligne : sinon O(clients × devis) à chaque render) les devis et l'**encaissé** de chaque client. ⚠ On somme `entryEncaisse`, **pas `e.ca`** : un devis au noir a `ca = 0` alors que l'argent est rentré. Le libellé dit donc « Encaissé », **jamais « CA »** (qui a un sens fiscal précis, cf. `bilanCompute`). `clientsOpenDevis()` pose le sticky **avant** `setTab("suivi")` — l'Historique ne liste que les devis clos, c'est l'exemption sticky qui rend visible un devis en cours.

- **Mission = formulaire à gauche + récap sticky à droite (`.ms-grid`)** — `rMissionAsideInner()` a **absorbé la toolbar** (`rMissionToolbarInner` supprimé, ainsi que `rMissionModeBar` → `rMissionModeCards`, et le `totalsHtml` mort). Il est ré-rendu en entier par `refreshDevisHeaders()` à chaque frappe ⇒ **il ne doit JAMAIS contenir d'`<input>`** (le focus serait perdu). Ses montants viennent de `totals()` et ses échéances de `t.echeances` (dérivées de `buildEcheanceLines`) — jamais un calcul parallèle. Statut = `validateDevis()`.
  - **Aperçu du devis à la demande** : `missionShowPreview` = variable module, **jamais dans `S`** (interaction purement visuelle).
  - **Sections du formulaire = cartes statiques TOUJOURS ouvertes** (maquette V2, arbitrage 2026-07-15 : l'accordéon repliable est ABANDONNÉ). `rEditSectionRaw(key, title, bodyHtml, numLabel)` rend une carte empilée (en-tête num + titre mono, corps visible). Machinerie supprimée : `_editSectionOpen`, `editSectionToggle`, `rEditSection` (non-Raw), `devisSectionSummaries` (puces de résumé) — `refreshDevisHeaders` ne re-rend plus que le récap sticky + les bannières. **Ne pas réintroduire d'accordéon ici.**
...
- **Boutons d'action = famille unique `.mk-btn`.** Le néon est **MORT** (phase 7 : `.btn-neon` / `.btn-add-neon` / `.ech-add-neon` supprimés, CSS compris) :
  ...
  - `.mk-btn.outline` (transparent + bordure `--border-light`, texte `--text-soft`) — secondaire NEUTRE (« Annuler » du récap Mission). ≠ `ghost`, qui est le contour EMBER des actions négatives.
  ...
  - `.mk-iconbtn.sm` / `.md` — boutons icône carrés (rotateCcw, moreVertical). Hover = `filter:brightness(1.09)`, active = `translateY(1px)`, `:disabled` = 45 %.
- **Status pill V2 = motif unique** (arbitrage 2026-07-14) : Space Mono 8.5px 700 uppercase, **fond transparent**, `border:1px solid <couleur à ~.33>`, radius 999px (`.dv-stage`, `.tr-badge`, `.cpt-role-pill`). Ne pas réintroduire de pill à fond teinté plein. Exceptions vivantes : badges de l'onglet Bugs (refonte complète à venir) et chips de filtre (fill = état actif).
- **Segmented controls `.seg-neon`** — la classe **garde son nom** ; l'option active se lit à son **fond plein azure** (`.warn` ambre pour Débours, `.ok` jade). **Style UNIQUE : tous les toggles sont en fill** (arbitrage 2026-07-15). La variante `.violet` (sélection en creux) est **supprimée** — la distinction données/présentation (arrondi + « Affichage sur le devis » en creux) n'était pas lisible et lue comme une incohérence. Ne pas la réintroduire.
- ⚠ **Un bouton porteur d'une icône ne se pilote JAMAIS par `textContent`** — piège systématique de la conversion glyphe → Lucide. `bugCopy`/`bugCopyOne` sauvegardaient/restauraient `textContent` pour flasher « Copié ! » : avec un SVG dedans, ça l'effaçait **définitivement** (jusqu'au prochain `render()`). Passer par `innerHTML`.
- **La règle « que du Lucide » vise l'UI, PAS les documents imprimés.** Les `✓` de « ✓ FACTURE SOLDÉE », « ✓ Soldé », « ✓ Justificatif joint » sont de la **typographie de document** : on les laisse. Les remplacer re-rendrait rétroactivement toutes les archives (cf. § bascule re-render).
...
- **Champ numérique avec unité** — pour tout input avec unité (`%`, `€`, `€/km`, `€/h`, `h`, `j`…), utiliser le pattern `.lock-field with-unit` : label SANS parens d'unité, unité dans un `<span class="lock-unit">…</span>` à droite de l'input, à l'intérieur de la même boîte visuelle. Si la valeur est un paramètre stable (taux barème, défaut métier), ajouter un bouton-menu `.lock-menu-btn` (icône `${ICO_SZ('moreVertical', 14)}`, plus jamais le caractère `⋮`) qui déverrouille le champ (pattern URSSAF / Marge avancés / Indemnité km). Sinon, l'unité est `:last-child` (coins arrondis à droite automatiques).
- **Label de zone hors d'un `.field`** : utiliser `.field-label-only` (11 px text-muted, même typo que les labels d'inputs — densifiés 13→11 px le 2026-07-14) plutôt qu'un `<h5>`. Le `<h5>` reste pour les vrais sous-titres à l'intérieur d'une section (désormais Space Mono 10 px 700 uppercase `.16em`).
...
**Parcours séquentiel verrouillé** (ordre canonique) : envoi (`dv.envoiTs`) → acceptation → par acompte dans l'ordre : paiement puis facture FA → **prestation réalisée** (`dv.livreeTs`) → facture FS → paiement solde → **envoi des photos** (`dv.photosTs`, jalon FINAL). ⚠ `livreeTs` = « prestation réalisée » (la séance faite, débloque FS), à NE PAS confondre avec `photosTs` = livraison des fichiers au client APRÈS paiement complet. `photosTs` est un marqueur **opérationnel** : il n'affecte NI le statut fiscal des entries (soldé = payé, `devisPrincipal` inchangé) NI le bilan — un devis payé mais photos non envoyées reste « terminé » côté fiscal, avec juste l'étape « Photos » active dans le parcours. **Panneau (refonte V2, phase 2 — TIMELINE VERTICALE)** : `devisParcoursSteps(devisId)` reste la SOURCE UNIQUE de la séquence + statut par étape (`done`/`active`/`upcoming`/`refused`). Elle est consommée par :
- `suiviParcoursHtml` → **timeline verticale** : une ligne par étape (pastille, libellé, sous-titre, montant, date d'encaissement éditable, chip facture, pastille d'état). Puis une **bande TERMINALE** (refus / annulation / clôture uniquement — les états en cours sont portés par l'étape active), les **frais & débours** (pas des étapes : aucune garde d'ordre n'en dépend), la régularisation.
- `suiviStepBack` (↺), rendu sur l'étape `backIdx` — **sauf en refus/annulation**, où la bande porte déjà un bouton explicite (sinon 2 contrôles pour la même action).
- `devisStageLabel(devisId)` → la pastille « où en est ce devis » des LIGNES (Accueil, Historique). **Dérivée, jamais un second calcul** : la ligne et la timeline ne peuvent pas se contredire.

⚠ **INVARIANT : les boutons d'action ne sont rendus QUE sur l'étape `active`** (`s.status === "active"`). C'est ce qui garantit un seul point d'émission de facture et interdit de court-circuiter une garde d'ordre depuis l'UI. Le chip facture vit sur l'étape FACTURE (plus sur la ligne d'échéance). `nextStepHint` supprimé. Gardes d'ordre dans `suiviLigneTogglePaiement`, `suiviMarkLivree` (exige acomptes réglés+facturés), `suiviMarkPhotos` (exige solde encaissé). `isFactureApplicable(solde)` dérive la chaîne amont COMPLÈTE (acomptes encaissés ET facturés + réalisée), pas juste les paiements.
...
**Un seul bouton d'archivage** : « Terminer », dans le **récap sticky** de Mission (`rMissionAsideInner`). Aucun bouton dans les previews live. Il route via `missionTerminer` → `suiviAdd()` (nouveau : crée les entries + `snapshotMission`) ou `suiviUpdateSnapshot()` (édition en place : met à jour snapshotMission, recrée les entries si manquantes).
...
Refonte livrée en 7 phases (2026-07-13 → 2026-07-14), validation utilisateur entre chaque. Maquette archivée : [refonte-v2/maquette-compact.dc.html](refonte-v2/maquette-compact.dc.html).
...
- [x] **Phase 6 — Profil + Bugs + Mon Compte** : onglet `compte` (`rCompte`) à sous-nav par chips, qui absorbe `rParametres` + `rMAJ` + corbeille + partage + sauvegarde + déconnexion ; Profil en cartes `.v2-panel` (+ templates Gmail) ; Notes désémojisées. Cf. § Page « Mon compte ».
- [x] **Phase 7 — Cleanup** : néon supprimé (31 boutons → `.mk-btn`, CSS compris), `.seg-neon` dégonflé, 11 derniers glyphes unicode → Lucide, **voile `.84` → `.75`** (toutes les vues sont en panneaux), maquette archivée dans [refonte-v2/](refonte-v2/).

**Refonte TERMINÉE.** La maquette est archivée et ne fait plus autorité : `index.html` + ce fichier sont la référence. Les **écarts assumés** (pas de toggle Tableau/Graphe en Compta, Clients en table + volet, Profil pas systématiquement en 2 colonnes, barèmes cession/matériel hors périmètre) sont consignés dans [refonte-v2/README.md](refonte-v2/README.md) — **ne pas les « corriger » en les prenant pour des oublis.**
...
`COMPTE_SECTIONS` = source unique de la sous-nav (chips). Section courante = `_compteSection`, **variable module, jamais dans `S`** (interaction purement visuelle). Sections : `general` (identité + préférences + déconnexion), `apparence`, `partage` (owner only, `ownerOnly:true`), `sauvegarde`, `corbeille`, `majlog`.

- **Garde owner** : le chip `partage` est filtré pour un membre, ET `rCompte` **retombe sur la 1ʳᵉ section valide** si `_compteSection` pointe sur une section devenue inaccessible (membre arrivant par un lien legacy) — sinon page blanche.
- **Le badge « N demandes en attente » vit sur l'onglet lui-même** (`.sb-user-menu-count`). Il était dans l'en-tête du menu supprimé : sans lui, l'owner n'a **aucun** signal qu'on attend son approbation. Il prime sur le dot « nouveautés ».
- **Curseur de voile** : `compteSetBgScrim` (UI) ≠ `setBgScrim` (console). ⚠ **Aucun `render()` dans son `oninput`** — re-rendre arracherait le curseur en plein glissement ; le `%` est rafraîchi à la main.
- **`binListHtml()` = source unique** de la liste de la corbeille générale, partagée par la section `corbeille` ET `rBinModal`. La **modale reste** pour un seul point d'entrée : le lien « Corbeille » de l'éditeur CGV — restaurer une section depuis là ne doit pas éjecter l'utilisateur du devis qu'il rédige.
- La corbeille des **devis** (`S.suivi.bin`) reste dans l'Historique ; la section pointe dessus au lieu de la dupliquer.

⚠ **Un bouton qui contient une icône ne se pilote plus par `textContent`** : `bugCopy`/`bugCopyOne` écrasaient le contenu du bouton pour flasher « Copié ! », puis le restauraient — avec un SVG dedans, ça l'effaçait **définitivement** (jusqu'au prochain `render()`). Passer par `innerHTML`. Piège systématique à la conversion glyphe → Lucide.
...
- **Smoke test hors navigateur (flux métier)** : `check.sh` ne valide que la syntaxe. Pour un flux à plusieurs étapes (archivage, parcours, gardes d'ordre, bilan), charger le **dernier** `<script>` inline d'`index.html` dans un contexte `vm` Node avec DOM/Firebase/localStorage bouchonnés (Proxy permissif + `firebase.auth.Auth.Persistence`), puis piloter `S` directement. ⚠ Les `const`/`let` top-level vivent dans la portée **lexicale** du contexte, PAS sur l'objet sandbox → les récupérer par `vm.runInContext("({S, totals, ...})")`. Les `function` top-level, elles, sont des props du global → `save = () => {}` les neutralise. Le sandbox doit exposer `window.addEventListener` (la 1ʳᵉ ligne du script pose un handler d'erreur global → `TypeError` immédiat sinon). A attrapé un bug bloquant (solde inencaissable) qu'aucune relecture n'avait vu. Script jetable dans le scratchpad, pas dans le repo.
- **Vérif visuelle sans se connecter** : copier `index.html` dans le scratchpad en injectant, **à la fin du script inline** (même portée lexicale — un `<script>` séparé ne voit ni `user` ni `workspaceState`), un harness qui pose `user` / `workspaceState.role = "owner"`, neutralise `save`/`saveToCloud`, sème des données dans `S`, puis appelle `render()`. ⚠ Firebase répond ensuite en asynchrone (`onAuthStateChanged` → `user = null`) et réaffiche le login par-dessus : réaffirmer l'état dans un `setInterval`. Capture ensuite en headless (`msedge --headless=new --screenshot`).
